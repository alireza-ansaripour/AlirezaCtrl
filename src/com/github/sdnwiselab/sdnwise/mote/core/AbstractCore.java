...
public abstract class AbstractCore {
    ...
    public void removeSendPacket(NetworkPacket packet){
        try {
            se.acquire();
            sendPackets.remove((int)packet.getMsgIndex());
            se.release();
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        ...
        for (Iterator<FlowTableEntry> it = flowTable.iterator();
            ...
            if (ttl != ENTRY_TTL_PERMANENT) {
                if (ttl >= ENTRY_TTL_DECR) {
                    ...
                    it.remove();
                    log(Level.INFO, "Removing rule"+ fte.toString() + " at position " + i);
                    if (i == 0) {
                        reset();
                    }
                }
     ...
     */
    public final void log(final Level level, final String logMessage) {
        try {
...
//        txQueue.add(np);
    ...
    private ArrayList<NetworkPacket>buffer = new ArrayList<>();
...
//            controllerTX(prepareReport());
    ...
    protected final void rxResponse(final ResponsePacket packet) {
        if (isAcceptedIdPacket(packet) && packet.getDst().equals(myAddress)) {
            ...
            for(int i =0 ; i< buffer.size();){
                if(matchRule(packet.getRule(), buffer.get(i))){
                    buffer.remove(i);
                }else
                    i++;
            }
    ...
    public NetworkPacket getPacket(int index) {
        try {
            se.acquire();
            NetworkPacket p =  sendPackets.get(index);
            se.release();
            return p;
        } catch (InterruptedException e) {
            e.printStackTrace();
